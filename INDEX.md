...
| 文章 | 来源项目 | 关键词 | 摘要 |
|------|----------|--------|------|
| [app-reverse-global-map.md](./mobile-app-reverse/app-reverse-global-map.md) | — (公众号/PDF 归档) | `App逆向`, `Android请求生命周期`, `OkHttp`, `Interceptor`, `JNI`, `SO层`, `Frida`, `SSL Pinning`, `设备指纹`, `VMP` | App 逆向全局地图：从 UI/业务/网络/加密/JNI/SO/网络发送七层拆解请求生命周期，总结定位加密点、还原算法、模拟请求三步法和常见防御手段 |
| [app-reverse-environment-setup.md](./mobile-app-reverse/app-reverse-environment-setup.md) | — (公众号/PDF 归档) | `App逆向环境`, `Android`, `LDPlayer`, `MuMu`, `Magisk`, `LSPosed`, `JustTrustMe`, `Charles`, `jadx-gui`, `APKTool`, `Frida`, `证书安装` | App 逆向环境搭建：模拟器与真机选型、Root/Magisk/LSPosed、证书安装与代理配置、抓包/反编译/动态调试工具链和常见问题排查 |
...
| [anti-crawler-app-reverse-series.md](./mobile-app-reverse/anti-crawler-app-reverse-series.md) | — (公众号归档) | `App逆向`, `jadx`, `Frida`, `JNI`, `SO层`, `密码算法`, `Python复现` | 从全局视角到 SO 层还原的 6 章 App 逆向入门系列；第1、2章与主文 app-reverse-global-map / app-reverse-environment-setup 去重后直接指向主文 |
...
| [uiautomator-privacy-consent-tap.md](./mobile-app-reverse/uiautomator-privacy-consent-tap.md) | — (知识星球归档) | `uiautomator`, `adb input tap`, `隐私协议`, `pm clear`, `设备注册`, `UI dump` | 首次启动隐私/权限弹窗：uiautomator dump 解析 bounds，对「同意/允许」中心点 `input tap`；界面问题不必先 Frida；来源更正为知识星球：逆向学习交流（2026-06-08），星球合集两份重复副本已并入 |
...
| [tiktok-xbogus-xgnarly-mstoken-env-patch.md](./web-reverse/tiktok-xbogus-xgnarly-mstoken-env-patch.md) | — (公众号归档) | `TikTok`, `X-Bogus`, `X-Gnarly`, `msToken`, `webmssdk.js`, `byted_acrawler`, `fetch 重写`, `补环境`, `report`, `strData`, `VMP 条件断点` | 無色逆向「TikTok逆向分析X-Bogus、X-Gnarly、msToken」：byted_acrawler.init 重写 window.fetch，hook setter 定位后补环境（约 200 行，canvas / toString）并赋值导出两个加密函数；msToken 由 report 响应头下发，strData 在 VMP 寄存器函数按长度条件断点收窄；35 张图已本地化 |
| [tencent-tdc-slider-vmp-part1-env-patch.md](./web-reverse/tencent-tdc-slider-vmp-part1-env-patch.md) | — (公众号归档) | `腾讯验证码`, `TCaptcha`, `TDC`, `tdc.js`, `jsvmp`, `cap_union_prehandle`, `cap_union_new_verify`, `collect`, `eks`, `补环境`, `RTCPeerConnection`, `createElement` | 無色逆向「腾讯滑块验证码vmp逆向分析-补环境篇」：prehandle → 动态 tdc.js → new_verify 参数表与错误码；collect = TDC.setData + getData(!0)；补环境难点在 RTCPeerConnection 原型、matchMedia、DOM 标签操作与 canvas/WebGL；collect 是否校验看客户配置；18 张图已本地化 |
| [tencent-tdc-slider-vmp-part2-tea-collect-pow.md](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md) | — (公众号归档) | `腾讯验证码`, `TDC`, `__TENCENT_CHAOS_VM`, `栈式虚拟机`, `插桩`, `TEA`, `XTEA`, `2654435769`, `>>> 5`, `collect`, `pow_answer`, `pow_calc_time`, `md5 PoW` | 無色逆向「腾讯滑块验证码vmp逆向分析-算法篇」：CHAOS_VM 结构剖析，apply/call 与运算插桩看出 4 字符分组 → TEA 家族（>>5 改 >>>5，key 选择同 XTEA）→ 乱码串拼接 base64；key 随 tdc.js 变，作者借 DeepSeek 从日志找 key；pow 为 md5(nonce+ans) 递增爆破；34 张图已本地化 |
| [hcaptcha-invisible-hsw-env-patch.md](./web-reverse/hcaptcha-invisible-hsw-env-patch.md) | — (公众号归档) | `hCaptcha`, `hsw.js`, `WASM`, `checksiteconfig`, `JWT`, `ArrayBuffer`, `motionData`, `n 值`, `补环境`, `RTCRtpSender`, `OfflineAudioContext`, `WebGL2`, `Math 精度`, `Worker` | 無色逆向「hCaptcha无感逆向分析-补环境」：Steam 注册页 demo；改 hcaptcha.html 让载荷走明文 JSON；n 值沿 Promise 链 → xr → hsw.js（解 JWT、aG 加载 WASM）；两处大数组插桩对照环境；九类检测点，作者补了三千多行；54 张图已本地化 |
| [aws-waf-token-challenge-purecalc.md](./web-reverse/aws-waf-token-challenge-purecalc.md) | — (公众号归档) | `AWS WAF`, `aws-waf-token`, `challenge.compact.js`, `verify`, `inputs`, `checksum`, `CRC32`, `Present`, `AES-GCM`, `solution`, `SHA256 PoW`, `纯算` | 無色逆向「某航司亚马逊aws-waf-token算法逆向分析」：token 由 verify 返回；checksum = CRC32(环境)，Present = base64(iv)::AES-GCM（固定 key、随机 iv），solution = inputs+checksum 的 SHA256 前 8 bit 为 0 工作量证明；Python 纯算跑通；25 张图已本地化 |
| [wasm-decrypt-ai-one-prompt-recovery.md](./web-reverse/wasm-decrypt-ai-one-prompt-recovery.md) | — (公众号归档) | `WASM`, `secure_decrypt_data`, `WebAssembly.Instance`, `同步加载`, `wasm2wat`, `wasm-decompile`, `AI 逆向`, `Cursor`, `Claude`, `SM2`, `SM4-ECB`, `C1C3C2` | 無色逆向「我用一句话让AI逆向出WASM中的加密算法」：招投标公告电话密文在 WASM 解密；先抠 JS 改同步加载本地 wasm 调用，再一句话让 Claude 反编译还原：SM2（C1C3C2，硬编码私钥）解出 SM4 key → SM4-ECB+PKCS7；约 15 分钟 / 300 万 token；30 张图已本地化 |
...
- **腾讯 tdc collect 魔改 TEA（2654435769 / `>>> 5`，key 随 tdc.js 变）+ pow md5(nonce+ans) 爆破**: [tencent-vmp-part2](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md), [tencent-captcha](./web-reverse/products/tencent-captcha.md)
- **AWS WAF aws-waf-token（checksum CRC32 / Present AES-GCM 固定 key 随机 iv / solution SHA256 前 8 bit 为 0 PoW）**: [aws-waf-token](./web-reverse/aws-waf-token-challenge-purecalc.md)
- **WASM 内 SM2（C1C3C2，硬编码私钥）解出 SM4 key → SM4-ECB + PKCS7**: [wasm-ai-sm2-sm4](./web-reverse/wasm-decrypt-ai-one-prompt-recovery.md)
...
- **腾讯 TCaptcha / TDC 补环境（cap_union 三接口 / RTCPeerConnection / DOM 标签操作 / canvas-WebGL；collect 校验看客户配置）**: [tencent-vmp-part1](./web-reverse/tencent-tdc-slider-vmp-part1-env-patch.md), [tencent-vmp-part2](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md), [tencent-captcha](./web-reverse/products/tencent-captcha.md)
- **hCaptcha 无感（hsw.js + WASM 出 n，ArrayBuffer 载荷改明文 JSON，RTCRtpSender / OfflineAudioContext / WebGL2 / Math 精度 / Worker 检测）**: [hcaptcha-hsw-env](./web-reverse/hcaptcha-invisible-hsw-env-patch.md)
- **AWS WAF challenge（aws-waf-token 403/401 判定、verify 纯算）**: [aws-waf-token](./web-reverse/aws-waf-token-challenge-purecalc.md)
- **TikTok Web X-Bogus / X-Gnarly / msToken（webmssdk.js 重写 fetch、report strData）**: [tiktok-xbogus-env](./web-reverse/tiktok-xbogus-xgnarly-mstoken-env-patch.md)
...
- **知识星球：逆向学习交流**: [xfq-crypto](./signature-algorithms/xfq-crypto-notes-compilation.md), [xfq-android-cases](./mobile-app-reverse/xfq-android-cases-compilation.md), [xfq-unidbg](./native-analysis/xfq-unidbg-native-compilation.md), [xfq-aosp-rom](./mobile-app-reverse/xfq-aosp-rom-compilation.md), [xfq-device-fp](./anti-detection/xfq-device-fp-compilation.md), [xfq-tools](./mobile-app-reverse/xfq-tools-debug-compilation.md), [reversenotes-android](./mobile-app-reverse/reversenotes-android-compilation.md), [uiautomator-consent](./mobile-app-reverse/uiautomator-privacy-consent-tap.md)
...
- **微信公众号：無色逆向 / TikTok · 腾讯滑块 vmp（上下篇）· hCaptcha · AWS WAF · WASM+AI**: [tiktok-xbogus-env](./web-reverse/tiktok-xbogus-xgnarly-mstoken-env-patch.md), [tencent-vmp-part1](./web-reverse/tencent-tdc-slider-vmp-part1-env-patch.md), [tencent-vmp-part2](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md), [hcaptcha-hsw-env](./web-reverse/hcaptcha-invisible-hsw-env-patch.md), [aws-waf-token](./web-reverse/aws-waf-token-challenge-purecalc.md), [wasm-ai-sm2-sm4](./web-reverse/wasm-decrypt-ai-one-prompt-recovery.md)
- **微信公众号：Softard（Wossoneri）**: [softard-android](./mobile-app-reverse/softard-android-reverse-compilation.md)
...
- **hCaptcha（hsw.js / checksiteconfig / Steam 注册页）**: [hcaptcha-hsw-env](./web-reverse/hcaptcha-invisible-hsw-env-patch.md)
- **Amazon AWS WAF（aws-waf-token / challenge.compact.js）**: [aws-waf-token](./web-reverse/aws-waf-token-challenge-purecalc.md)
...
- **腾讯（验证码/风控）**: [mmtls](./protocols/mmtls-protocol-analysis.md), [products](./web-reverse/products.md), [tencent-tdc-xtea](./web-reverse/tencent-tdc-slider-xtea-purecalc.md), [tencent-vmp-part1](./web-reverse/tencent-tdc-slider-vmp-part1-env-patch.md), [tencent-vmp-part2](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md)
...
- **hook `window.fetch` setter 定位 SDK 重写点 / 赋值到 window 导出 VMP 内加密函数 / VMP 寄存器函数按参数长度条件断点**: [tiktok-xbogus-env](./web-reverse/tiktok-xbogus-xgnarly-mstoken-env-patch.md)
- **栈式 VM（__TENCENT_CHAOS_VM）apply/call + 运算指令插桩、加法日志加条件；日志片段交给 AI 找 TEA key**: [tencent-vmp-part2](./web-reverse/tencent-tdc-slider-vmp-part2-tea-collect-pow.md)
- **Promise 链反跟定位异步入口 / WASM 交互大数组插桩对照补环境差异**: [hcaptcha-hsw-env](./web-reverse/hcaptcha-invisible-hsw-env-patch.md)
- **WASM 异步 instantiate 改 fs.readFileSync 同步加载 / 一句话让 Cursor + Claude 反编译 WASM 还原算法**: [wasm-ai-sm2-sm4](./web-reverse/wasm-decrypt-ai-one-prompt-recovery.md), [ai-assisted-web](./web-reverse/ai-assisted-web-reverse-compilation.md)
